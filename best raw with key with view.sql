...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 25, 2021 at 07:46 AM
...
--
-- Stand-in structure for view `view_februari`
-- (See below for the actual view)
--
CREATE TABLE `view_februari` (
`kode_obat` varchar(10)
,`nama_obat` varchar(15)
,`harga_satuan` int(15)
,`jumlah_terjual` int(5)
,`Total_Harga_Terjual` bigint(25)
,`sisa_stok` decimal(56,0)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `view_januari`
-- (See below for the actual view)
--
CREATE TABLE `view_januari` (
`kode_obat` varchar(10)
,`nama_obat` varchar(15)
,`harga_satuan` int(15)
,`jumlah_terjual` decimal(32,0)
,`Total_Harga_Terjual` decimal(46,0)
...
--
-- Stand-in structure for view `view_maret`
-- (See below for the actual view)
--
CREATE TABLE `view_maret` (
`kode_obat` varchar(10)
,`nama_obat` varchar(15)
,`harga_satuan` int(15)
,`jumlah_terjual` int(5)
,`Total_Harga_Terjual` bigint(25)
,`sisa_stok` decimal(57,0)
);

-- --------------------------------------------------------

--
-- Structure for view `view_februari`
--
DROP TABLE IF EXISTS `view_februari`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_februari`  AS  select `tabel_obat`.`Kode_Obat` AS `kode_obat`,`tabel_obat`.`Nama_Obat` AS `nama_obat`,`tabel_obat`.`Harga_Satuan` AS `harga_satuan`,`tabel_transaksi`.`Jumlah_Obat` AS `jumlah_terjual`,`tabel_obat`.`Harga_Satuan` * `tabel_transaksi`.`Jumlah_Obat` AS `Total_Harga_Terjual`,100 - ifnull(sum(`view_januari`.`jumlah_terjual`),0) - sum(`tabel_transaksi`.`Jumlah_Obat`) AS `sisa_stok` from (((`tabel_obat` left join `tabel_transaksi` on(`tabel_obat`.`Kode_Obat` = `tabel_transaksi`.`Kode_Obat`)) left join `tabel_persediaan` on(`tabel_persediaan`.`Kode_Obat` = `tabel_obat`.`Kode_Obat`)) left join `view_januari` on(`view_januari`.`kode_obat` = `tabel_obat`.`Kode_Obat`)) where month(`tabel_transaksi`.`Tgl_Transaksi`) = 2 group by `tabel_obat`.`Kode_Obat` ;

-- --------------------------------------------------------

--
-- Structure for view `view_januari`
--
DROP TABLE IF EXISTS `view_januari`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_januari`  AS  select `tabel_obat`.`Kode_Obat` AS `kode_obat`,`tabel_obat`.`Nama_Obat` AS `nama_obat`,`tabel_obat`.`Harga_Satuan` AS `harga_satuan`,sum(`tabel_transaksi`.`Jumlah_Obat`) AS `jumlah_terjual`,`tabel_obat`.`Harga_Satuan` * sum(`tabel_transaksi`.`Jumlah_Obat`) AS `Total_Harga_Terjual`,100 - sum(`tabel_transaksi`.`Jumlah_Obat`) AS `sisa_stok` from ((`tabel_obat` join `tabel_transaksi` on(`tabel_obat`.`Kode_Obat` = `tabel_transaksi`.`Kode_Obat`)) join `tabel_persediaan` on(`tabel_persediaan`.`Kode_Obat` = `tabel_obat`.`Kode_Obat`)) where month(`tabel_transaksi`.`Tgl_Transaksi`) = 1 group by `tabel_obat`.`Kode_Obat` ;

-- --------------------------------------------------------

--
-- Structure for view `view_maret`
--
DROP TABLE IF EXISTS `view_maret`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_maret`  AS  select `tabel_obat`.`Kode_Obat` AS `kode_obat`,`tabel_obat`.`Nama_Obat` AS `nama_obat`,`tabel_obat`.`Harga_Satuan` AS `harga_satuan`,`tabel_transaksi`.`Jumlah_Obat` AS `jumlah_terjual`,`tabel_obat`.`Harga_Satuan` * `tabel_transaksi`.`Jumlah_Obat` AS `Total_Harga_Terjual`,100 - ifnull(sum(`view_januari`.`jumlah_terjual`),0) - ifnull(sum(`view_februari`.`jumlah_terjual`),0) - sum(`tabel_transaksi`.`Jumlah_Obat`) AS `sisa_stok` from ((((`tabel_obat` left join `tabel_transaksi` on(`tabel_obat`.`Kode_Obat` = `tabel_transaksi`.`Kode_Obat`)) left join `tabel_persediaan` on(`tabel_persediaan`.`Kode_Obat` = `tabel_obat`.`Kode_Obat`)) left join `view_januari` on(`view_januari`.`kode_obat` = `tabel_obat`.`Kode_Obat`)) left join `view_februari` on(`view_februari`.`kode_obat` = `tabel_obat`.`Kode_Obat`)) where month(`tabel_transaksi`.`Tgl_Transaksi`) = 3 group by `tabel_obat`.`Kode_Obat` ;
...
ALTER TABLE `log_perubahan`
  ...
  ADD KEY `Log_Kode_Obat` (`Log_Kode_Obat`);

--
-- Indexes for table `obat_kadaluarsa`
--
ALTER TABLE `obat_kadaluarsa`
  ADD KEY `exp_Kode_Obat` (`exp_Kode_Obat`);
...
--
-- Indexes for table `tabel_persediaan`
--
ALTER TABLE `tabel_persediaan`
...
--
-- Constraints for table `log_perubahan`
--
ALTER TABLE `log_perubahan`
  ADD CONSTRAINT `log_perubahan_ibfk_1` FOREIGN KEY (`Log_Kode_Obat`) REFERENCES `tabel_obat` (`Kode_Obat`),
  ADD CONSTRAINT `log_perubahan_ibfk_2` FOREIGN KEY (`Log_Kode_Obat`) REFERENCES `tabel_obat` (`Kode_Obat`);

--
-- Constraints for table `obat_kadaluarsa`
--
ALTER TABLE `obat_kadaluarsa`
  ADD CONSTRAINT `obat_kadaluarsa_ibfk_1` FOREIGN KEY (`exp_Kode_Obat`) REFERENCES `tabel_obat` (`Kode_Obat`);

--
-- Constraints for table `orders`
--
ALTER TABLE `orders`
  ADD CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`kodeObat`) REFERENCES `tabel_obat` (`Kode_Obat`),
  ADD CONSTRAINT `orders_ibfk_2` FOREIGN KEY (`username`) REFERENCES `users` (`username`);

--
-- Constraints for table `tabel_persediaan`
--
ALTER TABLE `tabel_persediaan`
  ADD CONSTRAINT `tabel_persediaan_ibfk_1` FOREIGN KEY (`Kode_Obat`) REFERENCES `tabel_obat` (`Kode_Obat`);
COMMIT;
